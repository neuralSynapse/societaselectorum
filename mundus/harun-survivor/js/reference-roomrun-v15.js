...
const VERSION='15.0.0-contextual-workstations';
...
function floorColor(cell){
  ...
  if(cell.type==='market')return n>.5?'#574638':'#493a30';
...
function drawFloor(){
  ...
  for(const cell of floorCells){
    ...
    diamond(p.x,p.y,floorColor(cell),cell.type==='field'?'#29483c':cell.type==='market'?'#76604c':'#4d4c47');
...
const props=[
 ...
 {kind:'sacks',x:6.95,y:16.55,scale:.58},
 {kind:'sacks',x:16.85,y:14.45,scale:.52},
 {kind:'torch',x:3.1,y:14.1},{kind:'torch',x:10.8,y:18.5},{kind:'torch',x:18.1,y:13.7},{kind:'rune',x:15.9,y:16.1}
];
const workstations=[
 {id:'survival',x:4.65,y:18.15,name:'BANCADA DE SOBREVIVÊNCIA',short:'BANCADA',icon:'⚒',tone:'#a97843'},
 {id:'forge',x:17.55,y:15.75,name:'FORJA DE EXTRAÇÃO',short:'FORJA',icon:'◆',tone:'#cf7242'},
 {id:'field',x:8.15,y:12.55,name:'MESA AGRÁRIA',short:'CAMPO',icon:'☽',tone:'#aebc63'},
 {id:'precision',x:19.45,y:7.05,name:'MESA DE PRECISÃO',short:'PRECISÃO',icon:'✦',tone:'#c7aa68'}
];
const TOOL_STORAGE_KEY='harun.roomrun.tools.v15';
const TOOL_TREE=[
 {id:'knife',name:'FACA',icon:'†',station:'survival',level:1,cost:0,prev:null,desc:'FERRAMENTA INICIAL · CORTE E SOBREVIVÊNCIA'},
 {id:'axe',name:'MACHADO',icon:'⌁',station:'survival',level:1,cost:6,prev:'knife',desc:'MADEIRA E IMPACTO'},
 {id:'pickaxe',name:'PICARETA',icon:'⌃',station:'forge',level:2,cost:10,prev:'axe',desc:'PEDRA E VEIOS DUROS'},
 {id:'hammer',name:'MARTELO',icon:'⚒',station:'survival',level:2,cost:14,prev:'pickaxe',desc:'CONSTRUÇÃO E IMPACTO'},
 {id:'sickle',name:'FOICE',icon:'☽',station:'field',level:3,cost:18,prev:'hammer',desc:'COLHEITA RÁPIDA'},
 {id:'hoe',name:'ENXADA',icon:'⌐',station:'field',level:3,cost:22,prev:'sickle',desc:'CULTIVO E SOLO'},
 {id:'shovel',name:'PÁ',icon:'▽',station:'field',level:4,cost:28,prev:'hoe',desc:'ESCAVAÇÃO E PREPARO'},
 {id:'scythe',name:'GADANHA',icon:'ϟ',station:'field',level:5,cost:36,prev:'shovel',desc:'COLHEITA EM ARCO'},
 {id:'shears',name:'TESOURA',icon:'✂',station:'precision',level:6,cost:44,prev:'scythe',desc:'CORTE DE PRECISÃO'},
 {id:'drill',name:'BERBEQUIM',icon:'⌘',station:'forge',level:8,cost:60,prev:'shears',desc:'PERFURAÇÃO MECÂNICA'},
 {id:'harpoon',name:'ARPÃO',icon:'↟',station:'precision',level:10,cost:80,prev:'drill',desc:'ALCANCE E CAÇA'}
];
let toolState={owned:['knife'],active:'knife'};
try{const raw=JSON.parse(localStorage.getItem(TOOL_STORAGE_KEY)||'null');if(raw&&Array.isArray(raw.owned)){toolState.owned=[...new Set(['knife',...raw.owned])];toolState.active=toolState.owned.includes(raw.active)?raw.active:'knife'}}catch(_){}
let workbenchOpen=false,activeWorkbench=null,nearWorkbench=null;
const WORLD_MOTIFS=[
 {level:2,id:'war',title:'FRONTEIRA DE GUERRA',line:'Toda muralha cobra uma direção.'},
 {level:4,id:'cave',title:'A CAVERNA DAS SOMBRAS',line:'A sombra informa. A fonte decide.'},
 {level:6,id:'veil',title:'O VÉU MEDIADO',line:'Nem toda camada que organiza o real é o real.'},
 {level:8,id:'upstream',title:'CAPRA CONTRA A CORRENTE',line:'Subir exige distinguir resistência de direção.'}
];
const BLACK_BOX_PLAN=Object.freeze({
 status:'DORMANT_HOOK',
 activation:'evento estratégico posterior; nunca aleatório sem contexto',
 transition:['entrada','substância ficcional ritual','superposição','corredor','porta','missão','retorno'],
 doors:[
  {id:'game1',label:'PORTA I',route:null},
  {id:'game2',label:'PORTA II',route:null},
  {id:'game3',label:'PORTA III',route:null}
 ],
 rule:'serializar o estado do Jogo 4, assumir temporariamente as regras do mundo escolhido, cumprir uma missão e restaurar o estado ao retorno'
});
const acolytes=[
 {x:8.85,y:16.75,homeX:8.85,homeY:16.75,rescued:false,follow:0,bubble:'☉',cost:20,invest:0,padX:8.05,padY:17.65,roleName:'MOLEIRO'},
 {x:11.25,y:16.55,homeX:11.25,homeY:16.55,rescued:false,follow:1,bubble:'≡',cost:20,invest:0,padX:10.9,padY:17.55,roleName:'ESCRIBA'},
 {x:14.35,y:15.35,homeX:14.35,homeY:15.35,rescued:false,follow:2,bubble:'△',cost:40,invest:0,padX:14.15,padY:16.35,roleName:'GUARDIÃO'},
 {x:16.35,y:13.35,homeX:16.35,homeY:13.35,rescued:false,ambient:true,bubble:'◌'},
 {x:18.65,y:9.65,homeX:18.65,homeY:9.65,rescued:false,ambient:true,bubble:'⚗'}
];
const depot={x:6.35,y:15.55,input:0,process:0,name:'MOINHO DO LIMIAR'};
const harvestNodes=fieldCrops;
const builds=[
 {id:'butcher',x:10.75,y:14.45,cost:28,invest:0,done:false,name:'AÇOUGUE DO LIMIAR',height:0,tier:0,product:'meat',process:0,input:0},
 {id:'scriptorium',x:13.85,y:12.9,cost:68,invest:0,done:false,name:'SCRIPTORIUM',height:0,tier:0,product:'scroll',process:0,input:0},
 {id:'library',x:16.95,y:10.65,cost:145,invest:0,done:false,name:'BIBLIOTHECA',height:0,tier:0,product:'book',process:0,input:0},
 {id:'relicary',x:19.25,y:7.85,cost:280,invest:0,done:false,name:'RELICÁRIO',height:0,tier:0,product:'relic',process:0,input:0}
];
const marketGate={x:1.2,y:23.0};
const marketStalls=[
 {x:3.45,y:20.55,label:'BANCA I'},{x:6.45,y:21.15,label:'BANCA II'},{x:9.45,y:20.55,label:'BANCA III'}
];
...
function cutSpeed(){return 6.2*(1+upgrade('scythe').level*.34)*toolHarvestMultiplier()}
...
function beginNextLevel(){
  ...
  runState='playing';paused=false;UI.victory.hidden=true;last=performance.now();updateHUD();audio&&audio.setState('explore',{intensity:.4});const motif=motifForLevel(level);fx.push({kind:'banner',text:'ESTÁGIO '+level+(motif?' · '+motif.title:''),t:0,d:1.0});
}
function damageEnemy(e,dmg){
  if(e.dead||runState!=='playing')return;const power=(1+(level-1)*.035+metaLoot.arcana*.05)*toolCombatMultiplier();e.hp=Math.max(0,e.hp-dmg*power);e.hit=.11;emit('damage',e.x,e.y,{text:String(Math.round(dmg*power)),crit:dmg>22});
...
function updateAutomation(dt){
  const farm=upgrade('farm');
  if(farm.level>0){
    automation.harvestCd-=dt;
    if(automation.harvestCd<=0){
      const active=fieldCrops.filter(h=>h.amount>.02);
      if(active.length){
        const h=active[Math.floor(Math.random()*active.length)],take=Math.min(h.amount,1+Math.floor(farm.level/3));
        h.amount-=take;depot.input+=take;emit('pickup',h.x,h.y,{text:'AUTO +'+take+' FEIXE'});
        if(h.amount<=.02){h.amount=0;h.respawn=3.2+Math.random()*1.8}
      }
      automation.harvestCd=Math.max(.32,1.7-farm.level*.24);
    }
  }
  const guard=upgrade('guard');
  if(guard.level>0){
    automation.guardCd-=dt;
    if(automation.guardCd<=0){
      const live=enemies.filter(e=>!e.dead&&e!==boss);
      if(live.length){const e=live.reduce((a,b)=>dist(player,a)<dist(player,b)?a:b);damageEnemy(e,5+guard.level*3.2)}
      automation.guardCd=Math.max(.3,1.05-guard.level*.11);
    }
    if(guard.level>=2&&builds[0].done){
      for(const d of drops){
        if(d.dead||d.kind!=='meat')continue;
        const take=Math.min(d.value,dt*(.9+guard.level*.55));d.value-=take;builds[0].input+=take;
        if(d.value<=.05)d.dead=true;
      }
    }
  }
}
function updateUpgradePads(dt){
  for(const u of upgrades){
    if(u.level>=u.max||!upgradeAvailable(u))continue;
    if(dist(player,u)<.9&&coins>.01){
      const cost=upgradeCost(u),take=Math.min(coins,cost-u.invest,dt*20);coins-=take;u.invest+=take;updateHUD();
      if(Math.random()<dt*8)emit('spark',u.x,u.y,{});
      if(u.invest>=cost-.01){
        u.level++;u.invest=0;audio&&audio.sfx('level_up',{gain:.72,pitch:1.04+u.level*.025});shake=Math.max(shake,2.5);
        emit('banner',null,null,{text:u.name+' · NÍVEL '+u.level,d:.9});try{navigator.vibrate?.([7,18,9])}catch(_){}
      }
    }
  }
}
...
function interactionUpdate(dt){
  ...
  if(cutTarget){
    cutTarget.cut=(cutTarget.cut||0)+dt*cutSpeed();
    ...
    if(cutTarget.cut>=1){
      ...
      cutTarget.cut=0;
      if(cutTarget.amount<=.02){cutTarget.amount=0;cutTarget.respawn=Math.max(2.8,4.4-level*.04)+Math.random()*1.4;audio&&audio.sfx('room_clear',{gain:.24,pitch:1.12})}
      updateHUD();
    }
  }
...
function toolById(id){return TOOL_TREE.find(t=>t.id===id)||TOOL_TREE[0]}
function saveToolState(){try{localStorage.setItem(TOOL_STORAGE_KEY,JSON.stringify(toolState))}catch(_){}}
function ownsTool(id){return toolState.owned.includes(id)}
function toolEligible(t){return level>=t.level&&(!t.prev||ownsTool(t.prev))}
function toolHarvestMultiplier(){const m={knife:.82,axe:.94,pickaxe:.9,hammer:.9,sickle:1.16,hoe:1.24,shovel:1.08,scythe:1.48,shears:1.05,drill:.98,harpoon:1.02};return m[toolState.active]||1}
function toolCombatMultiplier(){const m={knife:1,axe:1.12,pickaxe:1.08,hammer:1.16,sickle:1.05,hoe:1.02,shovel:1.08,scythe:1.14,shears:1.04,drill:1.1,harpoon:1.18};return m[toolState.active]||1}
function stationTools(w){return TOOL_TREE.filter(t=>t.station===w.id)}
function nearestWorkstation(max=1.55){let best=null,bd=max;for(const w of workstations){const d=dist(player,w);if(d<bd){bd=d;best=w}}return best}
function workstationScreenPosition(w){
  const q=project(w.x,w.y),cx=W*.5,cy=H*.56,zx=cx+(q.x-cx)*sceneZoom,zy=cy+(q.y-cy)*sceneZoom,r=canvas.getBoundingClientRect();
  return{x:r.left+zx*r.width/W,y:r.top+zy*r.height/H}
}
function hitWorkbenchFromClient(x,y){
  if(!started||paused||workbenchOpen)return null;
  let best=null,bd=58;
  for(const w of workstations){if(dist(player,w)>1.75)continue;const p=workstationScreenPosition(w),d=Math.hypot(x-p.x,y-p.y);if(d<bd){bd=d;best=w}}
  return best
}
function ensureWorkbenchUi(){
  let prompt=document.getElementById('workbenchPromptV15');
  if(!prompt){prompt=document.createElement('button');prompt.id='workbenchPromptV15';prompt.className='workbenchPromptV15';prompt.type='button';prompt.innerHTML='<small>INTERAGIR</small><b>USAR BANCADA</b>';document.getElementById('app')?.appendChild(prompt);prompt.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(nearWorkbench)openWorkbench(nearWorkbench)})}
  let panel=document.getElementById('workbenchPanelV15');
  if(!panel){panel=document.createElement('section');panel.id='workbenchPanelV15';panel.className='workbenchPanelV15';panel.hidden=true;panel.innerHTML='<div class="workbenchCardV15"><header><div><small>INSTRUMENTA · OFÍCIO</small><h2 id="workbenchTitleV15">BANCADA</h2></div><button id="workbenchCloseV15" type="button" aria-label="Fechar">×</button></header><p id="workbenchLeadV15"></p><div id="workbenchToolsV15" class="workbenchToolsV15"></div><footer><span id="workbenchActiveV15"></span><b id="workbenchCoinsV15"></b></footer></div>';document.getElementById('app')?.appendChild(panel);panel.addEventListener('pointerdown',e=>e.stopPropagation());panel.addEventListener('click',e=>e.stopPropagation());panel.querySelector('#workbenchCloseV15').addEventListener('click',closeWorkbench)}
  return{prompt,panel}
}
function renderWorkbenchPanel(){
  if(!activeWorkbench)return;
  const {panel}=ensureWorkbenchUi(),tools=stationTools(activeWorkbench),title=panel.querySelector('#workbenchTitleV15'),lead=panel.querySelector('#workbenchLeadV15'),grid=panel.querySelector('#workbenchToolsV15');
  title.textContent=activeWorkbench.name;lead.textContent='Ferramentas aparecem aqui porque esta bancada tem função. Nada de enciclopédia grudada no HUD.';
  grid.innerHTML=tools.map(t=>{
    const owned=ownsTool(t.id),eligible=toolEligible(t),active=toolState.active===t.id,prev=t.prev?toolById(t.prev):null,afford=coins>=t.cost;
    const status=active?'EQUIPADA':owned?'EQUIPAR':!eligible?(level<t.level?'ESTÁGIO '+t.level:(prev?'REQUER '+prev.name:'SELADA')):'DESBLOQUEAR · ÓB '+t.cost;
    const disabled=!owned&&(!eligible||!afford);
    return '<button class="workbenchToolV15 '+(owned?'owned ':'')+(active?'active ':'')+'" data-tool="'+t.id+'" '+(disabled?'disabled':'')+'><i>'+t.icon+'</i><span><strong>'+t.name+'</strong><small>'+t.desc+'</small></span><em>'+status+'</em></button>'
  }).join('');
  panel.querySelectorAll('[data-tool]').forEach(btn=>btn.addEventListener('click',()=>{
    const t=toolById(btn.dataset.tool);
    if(ownsTool(t.id)){toolState.active=t.id;saveToolState();audio&&audio.sfx('instrumenta',{gain:.45,pitch:1.08});renderWorkbenchPanel();return}
    if(!toolEligible(t)||coins<t.cost)return;
    coins-=t.cost;toolState.owned.push(t.id);toolState.active=t.id;saveToolState();updateHUD();audio&&audio.sfx('level_up',{gain:.68,pitch:1.08});emit('banner',null,null,{text:t.name+' DESBLOQUEADA',d:.85});renderWorkbenchPanel()
  }));
  panel.querySelector('#workbenchActiveV15').textContent='ATIVA · '+toolById(toolState.active).name;
  panel.querySelector('#workbenchCoinsV15').textContent='ÓBOLOS '+Math.floor(coins);
}
function openWorkbench(w){
  if(!w||workbenchOpen||paused||runState!=='playing')return;
  ensureWorkbenchUi();activeWorkbench=w;workbenchOpen=true;endPointer();player.vx=player.vy=0;
  const p=document.getElementById('workbenchPromptV15');if(p)p.hidden=true;
  const panel=document.getElementById('workbenchPanelV15');panel.hidden=false;renderWorkbenchPanel();audio&&audio.sfx('instrumenta',{gain:.38,pitch:.94})
}
function closeWorkbench(){
  workbenchOpen=false;activeWorkbench=null;const panel=document.getElementById('workbenchPanelV15');if(panel)panel.hidden=true;last=performance.now();updateWorkbenchProximity()
}
function updateWorkbenchProximity(){
  const ui=ensureWorkbenchUi();nearWorkbench=(!workbenchOpen&&started&&!paused&&runState==='playing')?nearestWorkstation():null;
  ui.prompt.hidden=!nearWorkbench;
  if(nearWorkbench){ui.prompt.querySelector('b').textContent=nearWorkbench.short+' · USAR';ui.prompt.querySelector('small').textContent=viewportMode==='mobile'?'TOCAR':'E · OU CLIQUE NA MESA'}
}
function drawWorkstation(w){
  const p=project(w.x,w.y),near=nearWorkbench===w||activeWorkbench===w;
  ell(p.x,p.y+13,35,10,'#000',.3);
  poly([[p.x-34,p.y+4],[p.x+28,p.y-8],[p.x+38,p.y+2],[p.x-24,p.y+15]],'#261d18','#6d4d32',1.1);
  poly([[p.x-34,p.y-2],[p.x+28,p.y-14],[p.x+38,p.y-5],[p.x-24,p.y+8]],w.id==='forge'?'#3c3430':w.id==='field'?'#3b3824':w.id==='precision'?'#39332a':'#4c3524','#9a7448',1.2);
  line(p.x-24,p.y+8,p.x-24,p.y+24,'#211713',4);line(p.x+27,p.y-3,p.x+27,p.y+14,'#211713',4);
  if(w.id==='forge'){poly([[p.x-9,p.y-16],[p.x+9,p.y-20],[p.x+16,p.y-15],[p.x+2,p.y-10]],'#69635f','#b6a18a');glow(p.x+23,p.y-15,18,'#f0763c',.15)}
  else if(w.id==='field'){for(let i=0;i<4;i++)line(p.x-14+i*8,p.y-15,p.x-17+i*8,p.y-29,'#b39a3a',1.4,.9);ell(p.x+18,p.y-13,7,3,'#6c7844')}
  else if(w.id==='precision'){ctx.fillStyle='#d1ad63';ctx.font='16px serif';ctx.textAlign='center';ctx.fillText('⚙',p.x,p.y-14);line(p.x-20,p.y-11,p.x-7,p.y-24,'#bba06c',1.5)}
  else{line(p.x-20,p.y-12,p.x-5,p.y-25,'#a87843',2);line(p.x-4,p.y-15,p.x+14,p.y-22,'#c9b58d',1.8)}
  ctx.fillStyle=w.tone;ctx.font='900 13px serif';ctx.textAlign='center';ctx.fillText(w.icon,p.x,p.y-1);
  if(near){
    ctx.save();ctx.strokeStyle='#68e6aa';ctx.lineWidth=1.6;ctx.setLineDash([4,4]);ctx.beginPath();ctx.ellipse(p.x,p.y+8,39,15,0,0,TAU);ctx.stroke();ctx.restore();
    rr(p.x-47,p.y-57,94,17,8,'#070806e8','#6e9d72');ctx.fillStyle='#e9dfc3';ctx.font='800 7px Cinzel,serif';ctx.fillText(w.name,p.x,p.y-46)
  }
}
function motifForLevel(n){let out=null;for(const m of WORLD_MOTIFS)if(n>=m.level)out=m;return out}
function drawMotifLayer(){
  if(level>=2){const p=project(20.8,5.3);line(p.x,p.y,p.x,p.y-42,'#4c3024',4);poly([[p.x,p.y-40],[p.x+23,p.y-34],[p.x,p.y-24]],'#632d2b','#9a5144');line(p.x-17,p.y+8,p.x+18,p.y-10,'#5d5147',2,.7)}
  if(level>=4){const p=project(7.45,7.6);poly([[p.x-28,p.y+5],[p.x-19,p.y-29],[p.x+21,p.y-32],[p.x+31,p.y+4]],'#0c0d0c','#45433b');for(let i=0;i<3;i++)ell(p.x-12+i*12,p.y-6-i*2,6,12,'#000',.45)}
  if(level>=6){const p=project(18.9,11.2);ctx.save();ctx.globalAlpha=.28;ctx.fillStyle='#75d9a2';ctx.font='700 9px monospace';for(let i=0;i<6;i++)ctx.fillText(String.fromCharCode(0x16A0+(i*5)%70),p.x+(i%3)*11-11,p.y-20+(i*13+time*10)%46);ctx.restore()}
  if(level>=8){const p=project(21.0,8.6),k=Math.sin(time*1.3)*3;ell(p.x,p.y+7,10,4,'#000',.25);ell(p.x,p.y-2+k*.08,8,5,'#d7d0bb');ell(p.x+7,p.y-7+k*.08,5,4,'#d7d0bb');line(p.x+8,p.y-10,p.x+12,p.y-17,'#a99d85',1.6);line(p.x+5,p.y-10,p.x+3,p.y-17,'#a99d85',1.6);line(p.x-4,p.y+2,p.x-7,p.y+10,'#8b806e',2);line(p.x+4,p.y+2,p.x+8,p.y+10,'#8b806e',2)}
}
...
function updateFollowers(dt){
  ...
  rescuedList.forEach((a,i)=>{
    let target;
    if(i===0){target={x:depot.x-.72,y:depot.y+.38};a.role='MOLEIRO'}
    else if(i===1&&builds[1].done){target={x:builds[1].x-.72,y:builds[1].y+.42};a.role='ESCRIBA'}
    else{
      const moving=Math.hypot(player.vx,player.vy)>.12,dir=moving?Math.atan2(player.vy,player.vx):player.angle,back=1.18+(i===2?0:.38),side=(i-1)*.46;
      target={x:player.x-Math.cos(dir)*back-Math.sin(dir)*side,y:player.y-Math.sin(dir)*back+Math.cos(dir)*side};a.role='GUARDIÃO'
    }
    const dx=target.x-a.x,dy=target.y-a.y,m=Math.hypot(dx,dy);
    if(m<.055){a.x=target.x;a.y=target.y;return}
    const step=Math.min(m,dt*(m>1.4?5.0:3.25)),nx=a.x+dx/m*step,ny=a.y+dy/m*step;
    if(isWalkableRadius(nx,ny,.09)){a.x=nx;a.y=ny}
  });
}
function update(dt){
  time+=dt;if(shake>0)shake=Math.max(0,shake-dt*34);
  if(workbenchOpen){player.vx=player.vy=0;updateWorkbenchProximity();return}
  movePlayer(dt);updateWorkbenchProximity();interactionUpdate(dt);combatUpdate(dt);updateFollowers(dt);
...
function drawProp(p){
  ...
  else if(p.kind==='sacks'){const k=p.scale||.55;for(let i=0;i<3;i++)ell(s.x+(i-1)*9*k,s.y-Math.floor(i/2)*5*k,8*k,5*k,i%2?'#654238':'#735046',1,-.15)}
...
function drawMarketYard(){
  const gate=project(marketGate.x,marketGate.y);
  line(gate.x-22,gate.y+3,gate.x-22,gate.y-40,'#33251a',6);line(gate.x+22,gate.y+3,gate.x+22,gate.y-40,'#33251a',6);
  line(gate.x-22,gate.y-38,gate.x+22,gate.y-38,'#7e5b35',5);glow(gate.x,gate.y-37,25,'#e3b45c',.13);
  ctx.fillStyle='#e6c276';ctx.font='800 8px Cinzel,serif';ctx.textAlign='center';ctx.fillText('PORTA DO MERCADO',gate.x,gate.y-47);
  for(let i=0;i<marketStalls.length;i++){
    const s=marketStalls[i],p=project(s.x,s.y);
    poly([[p.x-28,p.y-3],[p.x+28,p.y-3],[p.x+23,p.y+9],[p.x-31,p.y+9]],'#4d3524','#9a7145',1.2);
    line(p.x-22,p.y+8,p.x-22,p.y+25,'#2b1d15',4);line(p.x+18,p.y+8,p.x+18,p.y+24,'#2b1d15',4);
    poly([[p.x-32,p.y-19],[p.x+32,p.y-19],[p.x+25,p.y-5],[p.x-25,p.y-5]],i%2?'#6e2f28':'#5d4930','#b78a4e',1);
    ctx.fillStyle='#f5ddb0';ctx.font='800 7px Cinzel,serif';ctx.fillText(s.label,p.x,p.y-25);
  }
}
function drawUpgradePad(u){
  const p=project(u.x,u.y),available=upgradeAvailable(u),maxed=u.level>=u.max,cost=upgradeCost(u),remain=Math.max(0,Math.ceil(cost-u.invest));
  ctx.save();ctx.setLineDash([4,4]);ctx.strokeStyle=maxed?'#62e6a5':available?'#e8c86a':'#68645d';ctx.lineWidth=1.5;ctx.beginPath();ctx.ellipse(p.x,p.y+3,31,13,0,0,TAU);ctx.stroke();ctx.restore();
  glow(p.x,p.y-7,available?19:10,maxed?'#61dca0':'#d8ad4d',available?.12:.04);
  ctx.fillStyle=maxed?'#80efba':available?'#f3d98b':'#88847d';ctx.font='900 13px serif';ctx.textAlign='center';ctx.fillText(u.icon,p.x,p.y+1);
  rr(p.x-34,p.y-42,68,17,7,'#070806e8',available?'#8b744d':'#484641');
  ctx.fillStyle=available?'#f2dfb0':'#918d85';ctx.font='800 7px Cinzel,serif';
  ctx.fillText(maxed?u.name+' MAX':u.name+' NV.'+u.level,p.x,p.y-31);
  if(!maxed){ctx.fillStyle='#d9b75e';ctx.font='800 6.5px Cinzel,serif';ctx.fillText(available?'ÓB '+remain:u.desc,p.x,p.y-20)}
}
function drawDepot(){
  ...
  ctx.fillStyle='#f0d36b';ctx.font='800 8px Cinzel,serif';ctx.textAlign='center';ctx.fillText('MOINHO · ◫ '+stock,p.x,p.y-32);
}
function drawBuild(b){
  ...
  if(!done){
    ...
    ctx.fillStyle=available?'#f3d883':'#8d918c';ctx.font='800 7.5px Cinzel,serif';ctx.textAlign='center';
...
function drawCarryStack(p,bob,side=-1){
  ...
  const sx=p.x+side*9,sy=p.y+4+bob;
  ctx.save();
  line(sx-1,sy-4,sx+side*2,sy-14,'#3d2a20',1.6,.9);
  ell(sx,sy-1,6.5,7.5,'#4b3528');ell(sx-side*1.5,sy-2,4.8,5.7,'#654735');
  line(sx-4,sy-5,sx+4,sy-5,'#a17b4d',1,.85);
  if(total>=Math.ceil(carryCapacity()*.8)){glow(sx,sy-3,10,'#dcb45d',.12);ctx.fillStyle='#f0d8a4';ctx.font='800 5px Cinzel';ctx.textAlign='center';ctx.fillText(String(total),sx,sy+1)}
  ctx.restore();
}
function heroFacing(){
...
function drawGuide(){
  if(!pointer.active)return;const raw=project(player.x,player.y),cx=W*.5,cy=H*.56,h={x:cx+(raw.x-cx)*sceneZoom,y:cy+(raw.y-cy)*sceneZoom},x=pointer.x,y=pointer.y;
...
function drawScene(){
  const cx=W*.5,cy=H*.56;ctx.save();ctx.translate(cx,cy);ctx.scale(sceneZoom,sceneZoom);ctx.translate(-cx,-cy);
  drawFloor();drawForest();drawWalls();drawMotifLayer();drawMarketYard();
  const drawables=[];
  ...
  workstations.forEach(w=>drawables.push({d:w.x+w.y+.018,fn:()=>drawWorkstation(w)}));
  ...
  drawFx();drawObjectiveBeacon();ctx.restore();drawGuide();
}
function loop(now){
...
canvas.addEventListener('pointerdown',e=>{e.preventDefault();if(workbenchOpen)return;const stationHit=hitWorkbenchFromClient(e.clientX,e.clientY);if(stationHit){openWorkbench(stationHit);return}if(!started||paused||runState!=='playing'||pointer.active)return;pointer.active=true;pointer.id=e.pointerId;pointer.box=pointer.boy=0;try{canvas.setPointerCapture?.(e.pointerId)}catch(_){}setJoy(e);audio&&audio.unlock()},{passive:false});
...
window.addEventListener('keydown',e=>{if(/^(INPUT|TEXTAREA|SELECT)$/.test(e.target?.tagName||''))return;const k=e.key.toLowerCase();if(['w','a','s','d','arrowup','arrowdown','arrowleft','arrowright'].includes(k))e.preventDefault();if(k==='e'&&!e.repeat&&nearWorkbench&&!workbenchOpen){e.preventDefault();openWorkbench(nearWorkbench);return}if(e.key==='Escape'&&!e.repeat){e.preventDefault();if(workbenchOpen){closeWorkbench();return}togglePause();return}if(!workbenchOpen)keys[k]=true},{passive:false});
...
function startGame(){
  started=true;paused=false;runState='playing';workbenchOpen=false;activeWorkbench=null;UI.start.hidden=true;UI.pause.hidden=true;UI.victory.hidden=true;idle=0;last=performance.now();audio&&audio.unlock();window.MUNDUSMusic?.setAudible?.();audio&&audio.setState('explore',{intensity:.38});resetRun();updateWorkbenchProximity();
}
...
function selfTest(){
  const checks={
    canvas:!!canvas&&!!ctx,
    workstations:workstations.length===4&&workstations.every((w,i)=>workstations.every((q,j)=>i===j||dist(w,q)>1.9)),
    tooling:TOOL_TREE.length===11&&ownsTool('knife')&&typeof openWorkbench==='function',
...
window.__HARUN_ROOMRUN_V15__={version:VERSION,start:startGame,reset:resetRun,selfTest,state:stateSnapshot,qa:qaTools,blackBoxPlan:BLACK_BOX_PLAN,tooling:{tree:TOOL_TREE,state:()=>({...toolState,owned:[...toolState.owned]}),workstations},sentinel:{gameId:'harun-survivor',mode:'contextual-workstations-v15',institutionalWrite:false,sourceArt:'native-in-engine-only',mobileFirst:true,desktopAdaptive:true,referenceStrict:true,paidAcolytes:true,freeAcolytes:false,simultaneousAcolytePads:true,stageLoop:true,allVisibleCropsHarvestable:true,contextualTools:true,legacyToolbelt:false,blackBox:'dormant-hook'}};
window.__HARUN_ROOMRUN_V14__=window.__HARUN_ROOMRUN_V15__;
window.__HARUN_ROOMRUN_V13__=window.__HARUN_ROOMRUN_V15__;
window.__HARUN_ROOMRUN_V12__=window.__HARUN_ROOMRUN_V15__;
window.__HARUN_ROOMRUN_V11__=window.__HARUN_ROOMRUN_V15__;
window.__HARUN_ROOMRUN_V10__=window.__HARUN_ROOMRUN_V15__;
window.__HARUN_ROOMRUN_V9__=window.__HARUN_ROOMRUN_V15__;
window.__MUNDUS_SENTINEL__=window.__MUNDUS_SENTINEL__||{};
window.__MUNDUS_SENTINEL__.roomrunV15=window.__HARUN_ROOMRUN_V15__;
