...
const SUPPORTED_EFFECT_KINDS := [
    "gauntlet", "adjustment", "aeon", "analysis_engine", "anchor", "anchor_stance", "armor_break", "art", "balanced_exchange", "belial_ground", "black_flame", "blessing", "blindspot_trade", "bond_pair", "break_the_map", "broken_crown", "chain_strike", "chariot", "choice_of_cups", "combo_engine", "command", "command_wave", "communion", "cone_fire", "craft_triangle", "curse", "curse_cleanse", "daimon_action", "dash_break", "death", "decan_modifier", "devil", "dominate", "door_cut", "dual_mark", "echo_card", "echo_instrument", "equitable_return", "fecundity", "feint", "fire_overflow", "flow_engine", "focus_spring", "focus_well", "formation_chain", "fortify", "fortress", "fortune", "grounding", "heal_link", "hermit", "ignite_seed", "investment", "lilith_veil", "line_beam", "line_of_truth", "lust", "manifest_field", "mark_reveal", "material_seed", "matrix_power", "matter_engine", "matter_overflow", "memory_recovery", "moon", "multiply_essence", "mutation_hook", "overdrive_risk", "overflow_heal", "pharmakon", "phase", "phosphoros_ember", "pleroma_balance", "precision", "precision_wager", "quiet_blade", "recovery_circle", "red_path", "reflect", "reflect_rule", "reroll", "resource_aura", "return", "reveal_all", "reveal_edge", "room_event", "route", "rupture_triangle", "saturn_cycle", "scarcity_alchemy", "secret_sight", "severance", "sigillum_contract", "silence", "slow_cut", "solar_burst", "split_will", "star", "still_pool", "suit_finisher", "sun", "suspension", "syzygy", "tempo_shift", "thoth_inscription", "threshold_gambit", "threshold_reset", "time_stop", "tower", "tradition", "transformation", "transmute_hurt", "universe", "velocity_engine", "victory_resonance", "war_stance", "ward", "will_culmination", "wound_to_focus"
]
...
func resolve(entry: Dictionary, context: Dictionary) -> Dictionary:
    ...
    if entry.is_empty(): result["blocked_by"].append("missing_entry"); return result
    var effect: Dictionary=entry.get("effect", {}); var kind:String=str(effect.get("kind", ""))
    if not SUPPORTED_EFFECT_KINDS.has(kind): result["blocked_by"].append("unsupported_effect:%s" % kind); return result
    var owned_ids:Array=context.get("owned_ids", []); var tags:Array=context.get("tags", [])
    ...
    if not _can_pay(entry.get("cost", {}), context.get("resources", {})): result["blocked_by"].append("insufficient_cost"); return result
    _record_cost(entry.get("cost", {}), result); _resolve_synergies(entry, owned_ids, tags, result); _resolve_stacking(entry, context.get("active_effects", {}), result)
    if not result["blocked_by"].is_empty(): return result
    _dispatch_effect(entry, effect, context, result)
    result["events"].append({"name":"effect.%s" % kind,"content_id":entry.get("id", ""),"effect":effect.duplicate(true),"vfx_hook":entry.get("vfx_hook", ""),"sfx_hook":entry.get("sfx_hook", "")}); result["consumed"]=entry.get("save_state", {}).get("scope", "run")!="none"; return result

func _can_pay(cost:Dictionary,resources:Dictionary)->bool:
    var resource:=str(cost.get("resource","none")); var amount:=float(cost.get("amount",0)); return resource=="none" or amount<=0 or float(resources.get(resource,0))>=amount
...
func _resolve_stacking(entry:Dictionary,active_effects:Dictionary,result:Dictionary)->void:
    var content_id:=str(entry.get("id","")); var stacking:Dictionary=entry.get("stacking",{}); var mode:=str(stacking.get("mode","none")); var max_stacks:=int(stacking.get("max_stacks",1)); var current:=int(active_effects.get(content_id,0))
    if mode in ["none","unique_transform"] and current>0: result["blocked_by"].append("stacking:%s"%mode); return
    var next_stack:=1
...
func _dispatch_effect(entry:Dictionary,effect:Dictionary,context:Dictionary,result:Dictionary)->void:
    ...
    match kind:
        ...
        "material_seed": result["state_changes"]["stat_delta:armor"]=float(effect.get("armor",0)); result["state_changes"]["resource_delta:essence"]=float(effect.get("essence",0))
        ...
        "route","curse","blessing","transformation","room_event","gauntlet": result["events"].append({"name":"meta.%s"%kind,"payload":effect.duplicate(true)})
