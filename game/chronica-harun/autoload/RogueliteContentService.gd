...
const REQUIRED_FIELDS := [
    "id", "name", "rarity", "pool", "eligibility", "effect", "cost", "duration",
    "stacking", "synergies", "exclusions", "vfx_hook", "sfx_hook", "save_state",
    "codex", "provenance"
]
...
func load_all() -> bool:
    ...
    for catalog_name in CATALOG_FILES:
        var records := _load_json_array(CATALOG_FILES[catalog_name])
        catalogs[catalog_name] = records
        for record in records:
            if not _validate_record(record, catalog_name):
                continue
            var record_id: String = str(record["id"])
            if by_id.has(record_id):
                load_errors.append("duplicate id: %s" % record_id)
                continue
...
func _load_json_array(path: String) -> Array:
    if not FileAccess.file_exists(path):
        load_errors.append("missing catalog: %s" % path)
        return []
    var text := FileAccess.get_file_as_string(path)
    var parsed = JSON.parse_string(text)
    if not parsed is Array:
        load_errors.append("catalog is not an array: %s" % path)
        return []
    return parsed

func _validate_record(record: Variant, catalog_name: String) -> bool:
    if not record is Dictionary:
        load_errors.append("%s contains a non-dictionary record" % catalog_name)
        return false
    for field in REQUIRED_FIELDS:
        if not record.has(field):
            load_errors.append("%s/%s missing %s" % [catalog_name, record.get("id", "?"), field])
            return false
    return true

func get_entry(content_id: String) -> Dictionary:
    return by_id.get(content_id, {})

func get_pool(pool: String) -> Array:
    return by_pool.get(pool, []).duplicate()

func eligible_entries(pool: String, context: Dictionary) -> Array:
    var result: Array = []
    var context_tags: Array = context.get("tags", [])
    var stage_index := int(context.get("stage_index", 0))
    var cycle := int(context.get("cycle", 0))
    for record in get_pool(pool):
        var eligibility: Dictionary = record.get("eligibility", {})
        if stage_index < int(eligibility.get("min_stage", 0)):
            continue
        if cycle < int(eligibility.get("min_cycle", 0)):
            continue
        var requires: Array = eligibility.get("requires_tags", [])
        var forbids: Array = eligibility.get("forbids_tags", [])
        if not _contains_all(context_tags, requires):
            continue
        if _contains_any(context_tags, forbids):
            continue
