...
CATALOG_FILES = {
    "tarot":"tarot.json", "sigilla":"sigilla.json", "pharmaka":"pharmaka.json",
    "talismans":"talismans.json", "instrumenta":"instrumenta.json", "powers":"powers.json",
    "mutations":"mutations.json", "daimones":"daimones.json", "relics":"relics.json",
    "transformations":"transformations.json", "curses":"curses.json", "blessings":"blessings.json",
    "routes":"routes.json", "special_rooms":"special_rooms.json",
}
...
EXPECTED = {"tarot":78,"sigilla":72,"pharmaka":21,"talismans":36,"instrumenta":32,"powers":15,"mutations":45,"daimones":7,"relics":9,"transformations":12,"curses":8,"blessings":6,"routes":8,"special_rooms":20}

@pytest.fixture
def catalogs():
    return {k: json.loads((DATA/v).read_text(encoding="utf-8")) for k,v in CATALOG_FILES.items()}
...
def test_all_records_have_common_runtime_contract(catalogs):
    for key, records in catalogs.items():
        for record in records:
            ...
            assert not missing, (key, record.get("id"), sorted(missing))

def test_ids_are_globally_unique(catalogs):
    ids=[r["id"] for records in catalogs.values() for r in records]
...
def test_every_record_has_runtime_hooks_and_save_contract(catalogs):
    for records in catalogs.values():
        for r in records:
            ...
            assert r["vfx_hook"].startswith("vfx.")
            assert r["sfx_hook"].startswith("sfx.")
...
def test_meta_counts_meet_minimums(catalogs):
    assert len(catalogs["relics"]) >= 9
    assert len(catalogs["transformations"]) >= 10
    assert len(catalogs["curses"]) >= 8
    assert len(catalogs["blessings"]) >= 6
