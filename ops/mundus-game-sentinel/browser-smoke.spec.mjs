...
const cases=[
  {id:'chronica-3d',route:'/chronica-harun-3d.html',viewport:{width:1440,height:900},start:'#action',settle:9000},
  {id:'harun-roguelite',route:'/harun-roguelite.html',viewport:{width:1280,height:720},start:'#start',settle:2500},
  {id:'harun-survivor',route:'/harun-survivor.html',viewport:{width:390,height:844},start:'#startQuick',settle:2500},
];
for(const cfg of cases){
  test(`${cfg.id} boots and accepts primary input`,async({browser})=>{
    ...
    await page.screenshot({path:`artifacts/${cfg.id}-boot.png`,fullPage:true}); const start=page.locator(cfg.start);
    ...
    await page.screenshot({path:`artifacts/${cfg.id}-after-input.png`,fullPage:true}); const criticalFailed=failed.filter(x=>/chronica-|harun-|three|cdn\.websitepublisher/i.test(x));
